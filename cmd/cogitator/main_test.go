...
// idsAfterSort is a small helper that runs sortLiveRows and returns
// the session IDs in their post-sort order, which is what every
// ordering test below actually wants to assert on.
...
// TestSortLiveRowsByCreatedAsc is the baseline: three idle rows differ
// only in Created, so they must come out oldest-first.
...
// TestSortLiveRowsAttentionPinsAboveBand pins the band rule: a newer
// permission-pending row outranks an older inactive row.
...
// TestSortLiveRowsAllUpperBandAttentionsPinned makes sure every
// Attention with Rank() < 2 (Permission, Question, Errored) lands in
// the upper band. A regression that, say, tightened the threshold to
// Rank() < 1 would silently demote Errored — this test catches that.
...
// TestSortLiveRowsActiveNotPinned protects the explicit "2 bands"
// decision: AttnActive (busy) must NOT jump above AttnInactive,
// because we don't want rows to dance on busy↔idle flicker.
...
// TestSortLiveRowsAttentionBandSortedByCreated confirms the secondary
// sort holds inside the pinned band: two permission rows still come
// out oldest-first.
...
// TestSortLiveRowsZeroCreatedFallback covers the both-zero case: when
// no row has a known initiation time, fall back to LastActivity DESC
// so freshly-touched rows still float to the top instead of all
// colliding at the epoch.
...
// TestSortLiveRowsZeroVsNonZero pins the mixed case: within a band,
// a row whose Created has been resolved outranks one still waiting on
// its /session/{id} fetch.
func TestSortLiveRowsZeroVsNonZero(t *testing.T) {
	created := time.Unix(5_000, 0)
	activity := time.Unix(9_000, 0) // newer than the resolved row
	rows := []state.SessionView{
...
// TestSortRecentRowsByLastActivityDesc pins the contract that the
// recent block is NOT subject to the live-block policy. Recent rows
// keep their historical "most recently touched first" order — when
// this test fails it means someone routed recent rendering through
// sortLiveRows again, which would reorder the recent section by
// Created and surprise the user.
func TestSortRecentRowsByLastActivityDesc(t *testing.T) {
	...
	// The newer-LastActivity row has the newer Created too, so if the
	// live policy were applied it'd land last (Created ASC). Asserting
	// it lands first proves we're on the recent policy.
	rows := []state.SessionView{
...
// TestRenderTreeAppliesSortToKids is the integration check: the same
// policy that orders roots must order the per-parent kids slice too,
// including the attention pin.
func TestRenderTreeAppliesSortToKids(t *testing.T) {
	...
	// kid-perm must appear before kid-old in the rendered output even
	// though kid-old has the older Created.
